...
export class LoginComponent implements OnInit {
  ...
  onLogin() {
    this.loginService.logInAccountRequest(this.loginObj).subscribe(
      (res) => {
        if (res.status === Constant.USER_STATUS.INACTIVE) {
          alert(
            "Your account is currently inactive. Please contact your leader for assistance."
          );
          return;
        }

        if (res.status === Constant.USER_STATUS.CLOSED) {
          alert(
            "Your account is currently closed. Please contact your leader for assistance."
          );
          return;
        }
